...
class RegRBM(RBM):
    ...
    def _initialize_vars(self):
        ...
        reg_w_grad = \
            tf.reduce_mean(
                (sample_bernoulli(hidden_p) / tf.sqrt(2 * np.pi)) * \
                tf.expand_dims(tf.reduce_sum(
                    tf.exp(
                        -1 * (2 * tf.square(self.sigma)) * \
                        tf.square(self.t - tf.matmul(sample_bernoulli(hidden_p), tf.transpose(self.w)) - self.visible_bias)),
                    axis=1), 1),
                axis=0)
        reg_visible_bias_grad = \
            tf.reduce_mean(
                (1 / self.sigma * tf.sqrt(2 * np.pi)) * \
                tf.expand_dims(tf.reduce_sum(
                    tf.exp(
                        -1 * (2 * tf.square(self.sigma)) * \
                        tf.square(self.t - tf.matmul(sample_bernoulli(hidden_p), tf.transpose(self.w)) - self.visible_bias)),
        ...
        delta_w_new = f(self.delta_w, positive_grad - negative_grad - self.lam * reg_w_grad)
        delta_visible_bias_new = f(self.delta_visible_bias, tf.reduce_mean(self.x - visible_recon_p, 0) - self.lam * reg_visible_bias_grad)
